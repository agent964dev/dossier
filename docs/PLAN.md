...
- A sandboxed document can still assign `location` and navigate the reader's top-level tab to an external URL (proven in Chromium 147 on 2026-09-12; meta refresh is already blocked). The plan deliberately serves documents top-level so links and the hub work; a first-party viewer iframe was tried in phase 3 and reverted because it broke link navigation. Owner decision pending on whether to block script-driven top-level navigation (would need the viewer approach with `allow-top-navigation-by-user-activation` semantics) or accept it as equivalent to a clicked link.
