...
async function resolve(token: string): Promise<PrincipalIdentity> {
  return run(Effect.gen(function* () {
    ...
    return yield* principals.resolve(new Request('https://dossier.test/api/me', {
...
async function actor(
  ...
  const seeded = await seedPrincipal(env, { suffix, email: email?.verified ? email.value : undefined })
  if (email && !email.verified) {
    ...
    ).bind(`identity_unverified_${suffix}`, seeded.accountId, `unverified_${suffix}`, email.value, now, now).run()
  }
  if (targetWorkspace && role) {
    await env.DB.prepare(
      `INSERT INTO memberships (workspace_id, account_id, role, created_at)
       VALUES (?, ?, ?, ?)`,
    ).bind(targetWorkspace, seeded.accountId, role, now).run()
    await env.DB.prepare(`UPDATE api_keys SET workspace_id = ? WHERE id = ?`)
      .bind(targetWorkspace, seeded.keyId).run()
  }
  return { principal: await resolve(seeded.token), token: seeded.token, accountId: seeded.accountId }
}

function publish(
  principal: PrincipalIdentity,
  title: string,
  options: Partial<Parameters<import('../src/services').PublishService['publish']>[0]> = {},
) {
  return run(Effect.gen(function* () {
...
function patchEither(documentId: string, body: Parameters<import('../src/services').TreeService['patch']>[1], principal: PrincipalIdentity) {
  return run(Effect.gen(function* () {
...
describe('phase two tree and access', () => {
  it('enforces public, team, private, invite, verification, and inherited boundaries', async () => {
    const ownerSeed = await seedPrincipal(env, { suffix: 'acl_owner', email: 'owner@acl.test' })
    ...
    const invited = await actor('acl_invited', undefined, undefined, { value: 'guest@outside.test', verified: true })
    const unverified = await actor('acl_unverified', undefined, undefined, { value: 'guest@outside.test', verified: false })
    const outsider = await actor('acl_outsider')

    const roots = {
      public: await publish(owner, 'ACL public', { visibility: 'public', shares: ['guest@outside.test'] }),
      team: await publish(owner, 'ACL team', { visibility: 'team', shares: ['guest@outside.test'] }),
      private: await publish(owner, 'ACL private', { visibility: 'private', shares: ['guest@outside.test'] }),
    }
    const children = {
      public: await publish(owner, 'ACL public inherited', { parentId: roots.public.document.id }),
      team: await publish(owner, 'ACL team inherited', { parentId: roots.team.document.id }),
      private: await publish(owner, 'ACL private inherited', { parentId: roots.private.document.id }),
    }
    ...
    for (const visibility of ['public', 'team', 'private'] as const) {
      for (const target of [roots[visibility], children[visibility]]) {
        ...
        expect((await access(target.document.id, admin.principal)).canRead).toBe(true)
        expect((await access(target.document.id, member.principal)).canRead).toBe(visibility !== 'private')
        expect((await access(target.document.id, invited.principal)).canRead).toBe(true)
        expect((await access(target.document.id, unverified.principal)).canRead).toBe(visibility === 'public')
        expect((await access(target.document.id, outsider.principal)).canRead).toBe(visibility === 'public')
        expect((await access(target.document.id, null)).canRead).toBe(visibility === 'public')
      }
      expect((await access(children[visibility].document.id, owner)).accessSource).toBe('inherited')
    }
  })
  ...
  it('treats a public child under a hidden parent as a virtual root without leaking ancestors', async () => {
    ...
    const hidden = await publish(owner, 'Never reveal this ancestor title', { visibility: 'private' })
    const sibling = await publish(owner, 'Never reveal this sibling title', { parentId: hidden.document.id, visibility: 'private' })
    const child = await publish(owner, 'Visible child', { parentId: hidden.document.id, visibility: 'public' })

    const tree = await run(Effect.gen(function* () {
    ...
    const detail = await run(Effect.gen(function* () {
      return yield* (yield* Documents).get(child.document.id, outsider.principal)
    }))
    ...
    const hiddenParentList = await run(Effect.gen(function* () {
      return yield* (yield* Documents).list(
        { scope: 'readable', parent: hidden.document.id },
        outsider.principal,
      ).pipe(Effect.either)
    }))
    expect(hiddenParentList).toMatchObject({ _tag: 'Left', left: { code: 'not_found' } })
  })

  it('filters and orders virtual roots without consulting hidden storage ancestry', async () => {
    const ownerSeed = await seedPrincipal(env, { suffix: 'virtual_order_owner' })
    const owner = await resolve(ownerSeed.token)
    ...
    expect(
      tree.documents
        .filter((document) => document.id === alpha.document.id || document.id === zulu.document.id)
    ...
    const roots = await run(Effect.gen(function* () {
      return yield* (yield* Documents).list(
  ...
  it('uses the current boundary for historical serving and node-only availability', async () => {
    ...
    const owner = await resolve(ownerSeed.token)
    const parent = await publish(owner, 'Disabled ancestor', { visibility: 'private' })
    const document = await publish(owner, 'Historical ACL', { parentId: parent.document.id, visibility: 'public' })
    const serving = () => run(Effect.gen(function* () {
      return yield* (yield* Serving).serve(new Request(`${document.publicUrl}/v/1`))
    }))
    ...
    await run(Effect.gen(function* () {
      yield* (yield* Tree).patch(document.document.id, { visibility: 'private' }, owner)
    }))
    expect((await serving()).status).toBe(404)
    await run(Effect.gen(function* () {
      yield* (yield* Tree).patch(document.document.id, { visibility: 'public' }, owner)
  ...
  it('creates through depth 16 and rejects deeper creates, cycles, overflow, and cross-workspace parents', async () => {
    ...
    for (let depth = 1; depth <= 16; depth += 1) {
      chain.push(await publish(owner, `Depth ${depth}`, { parentId: chain.at(-1)!.document.id }))
    }
    const tooDeep = await run(Effect.gen(function* () {
      return yield* (yield* Publish).publish({
        html: html('Depth 17'),
        parentId: chain.at(-1)!.document.id,
        idempotencyKey: 'depth-17-rejected',
      }, owner).pipe(Effect.either)
    }))
    expect(tooDeep).toMatchObject({ _tag: 'Left', left: { code: 'policy_rejected' } })

    expect(await patchEither(chain[0].document.id, { parentId: chain[1].document.id }, owner))
      .toMatchObject({ _tag: 'Left', left: { code: 'policy_rejected' } })
    const movable = await publish(owner, 'Movable root child', { parentId: chain[0].document.id })
    expect(await patchEither(movable.document.id, { parentId: chain.at(-1)!.document.id }, owner))
      .toMatchObject({ _tag: 'Left', left: { code: 'policy_rejected' } })

    const otherSeed = await seedPrincipal(env, { suffix: 'depth_other' })
    const other = await resolve(otherSeed.token)
    const otherRoot = await publish(other, 'Other workspace public', { visibility: 'public' })
    expect(await patchEither(movable.document.id, { parentId: otherRoot.document.id }, owner))
      .toMatchObject({ _tag: 'Left', left: { code: 'not_found' } })

    const rooted = await run(Effect.gen(function* () {
      return yield* (yield* Tree).patch(movable.document.id, { parentId: null }, owner)
    }))
    expect(rooted.parentId).toBeNull()
    const storage = await env.DB.prepare(`SELECT path, depth FROM documents WHERE id = ?`)
      .bind(movable.document.id).first<{ path: string; depth: number }>()
  ...
  it('rewrites tombstone paths and enforces admin editor overrides', async () => {
    ...
    const root = await publish(member.principal, 'Member document', { visibility: 'team' })
    const tombstone = await publish(member.principal, 'Moved tombstone', { parentId: root.document.id })
    await run(Effect.gen(function* () {
      yield* (yield* Documents).delete(tombstone.document.id, member.principal)
    }))
    const destination = await publish(owner, 'Admin destination', { visibility: 'team' })
    const before = await env.DB.prepare(`SELECT path FROM documents WHERE id = ?`)
      .bind(tombstone.document.id).first<{ path: string }>()

    const memberDenied = await patchEither(destination.document.id, { description: 'member edit' }, member.principal)
    expect(memberDenied).toMatchObject({ _tag: 'Left', left: { code: 'editor_required' } })
    const changed = await run(Effect.gen(function* () {
      return yield* (yield* Tree).patch(root.document.id, {
        parentId: destination.document.id,
        kind: '  ReSearch-Note  ',
      }, admin.principal)
    }))
    expect(changed.kind).toBe('research-note')
    const after = await env.DB.prepare(`SELECT path FROM documents WHERE id = ?`)
      .bind(tombstone.document.id).first<{ path: string }>()
    ...
    const deleted = await run(Effect.gen(function* () {
      return yield* (yield* Documents).delete(root.document.id, admin.principal, true)
    }))
    expect(deleted.deleted).toBe(1)
  })

  it('archives the current parent graph when stored paths no longer match a preflight view', async () => {
    const ownerSeed = await seedPrincipal(env, { suffix: 'archive_current_tree_owner' })
    const owner = await resolve(ownerSeed.token)
    const root = await publish(owner, 'Archive current tree root', { visibility: 'team' })
    ...
    await env.DB.prepare(`UPDATE documents SET path = '/stale/', depth = 1 WHERE id = ?`)
      .bind(root.document.id)
      .run()

    const refused = await run(Effect.gen(function* () {
      return yield* (yield* Documents).delete(root.document.id, owner).pipe(Effect.either)
    }))
    ...
    ).bind(root.document.id, child.document.id).all<{ id: string; deleted_at: string | null }>()
    expect(beforeForce.results.every((row) => row.deleted_at === null)).toBe(true)

    const deletion = await run(Effect.gen(function* () {
    ...
    ).bind(deletion.batchId).all<{ id: string }>()
  ...
  it('materialises inherited shares, composes concurrent deltas, and rejects stale replacement', async () => {
    ...
    const child = await publish(owner, 'Inheriting child', { parentId: parent.document.id })

    await Promise.all([
      run(Effect.gen(function* () {
        return yield* (yield* Shares).delta(child.document.id, { add: ['one@example.test'] }, owner)
      })),
      run(Effect.gen(function* () {
        return yield* (yield* Shares).delta(child.document.id, { add: ['two@example.test'] }, owner)
      })),
    ])
    ...
    const revision = (await env.DB.prepare(`SELECT revision FROM documents WHERE id = ?`)
      .bind(child.document.id).first<{ revision: number }>())!.revision
    await run(Effect.gen(function* () {
      yield* (yield* Shares).replace(child.document.id, {
        emails: ['replacement@example.test'],
        ifRevision: revision,
      }, owner)
    }))
    const stale = await run(Effect.gen(function* () {
      return yield* (yield* Shares).replace(child.document.id, {
  ...
  it('archives multi-author live subtrees and restores only the current batch', async () => {
    ...
    const parent = await publish(owner, 'Archive parent', { visibility: 'team' })
    const ticket = await publish(owner, 'Archive ticket', { parentId: parent.document.id })
    const old = await publish(owner, 'Old tombstone', { parentId: ticket.document.id })
    const oldDelete = await run(Effect.gen(function* () {
      return yield* (yield* Documents).delete(old.document.id, owner)
    }))
    const research = await publish(member.principal, 'Intern research', { parentId: ticket.document.id })

    const conflict = await run(Effect.gen(function* () {
      return yield* (yield* Documents).delete(ticket.document.id, owner).pipe(Effect.either)
    }))
    ...
    expect(deletion.authors).toEqual(expect.arrayContaining([
      ...
      { accountId: member.accountId, name: member.principal.accountName, count: 1 },
    ]))

    const ownerTrash = await run(Effect.gen(function* () {
      return yield* (yield* Documents).list({ scope: 'trash' }, owner)
    }))
    const ticketRoot = ownerTrash.documents.find((document) => document.id === ticket.document.id)
    expect(ticketRoot && isDocumentEditor(ticketRoot) ? ticketRoot.deletionRootTitle : null)
      .toBe('Archive ticket')
    expect(ticketRoot && isDocumentEditor(ticketRoot) ? ticketRoot.deletedBy : null)
      .toBe(owner.accountName)
    expect(ticketRoot && isDocumentEditor(ticketRoot) ? ticketRoot.authors : null)
      .toEqual(expect.arrayContaining([
        { accountId: owner.accountId, name: owner.accountName, count: 1 },
        { accountId: member.accountId, name: member.principal.accountName, count: 1 },
      ]))
    const memberTrash = await run(Effect.gen(function* () {
      return yield* (yield* Documents).list({ scope: 'trash' }, member.principal)
    }))
    expect(memberTrash.documents).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ id: research.document.id }),
    ]))

    const oldState = await env.DB.prepare(`SELECT deletion_batch_id FROM documents WHERE id = ?`)
      .bind(old.document.id).first<{ deletion_batch_id: string }>()
    expect(oldState?.deletion_batch_id).toBe(oldDelete.batchId)

    const parentDelete = await run(Effect.gen(function* () {
      return yield* (yield* Documents).delete(parent.document.id, owner)
    }))
    const refused = await run(Effect.gen(function* () {
      return yield* (yield* Documents).restore(ticket.document.id, deletion.batchId, owner).pipe(Effect.either)
    }))
    expect(refused).toMatchObject({ _tag: 'Left', left: { code: 'conflict' } })
    await run(Effect.gen(function* () {
      yield* (yield* Documents).restore(parent.document.id, parentDelete.batchId, owner)
      yield* (yield* Documents).restore(ticket.document.id, deletion.batchId, owner)
    }))
    const states = await env.DB.prepare(
      `SELECT id, deleted_at, deletion_batch_id FROM documents
        WHERE id IN (?, ?, ?) ORDER BY id`,
    ).bind(ticket.document.id, research.document.id, old.document.id).all<{
      id: string
      deleted_at: string | null
      deletion_batch_id: string | null
    }>()
    expect(states.results.find((row) => row.id === ticket.document.id)?.deleted_at).toBeNull()
    expect(states.results.find((row) => row.id === research.document.id)?.deleted_at).toBeNull()
    expect(states.results.find((row) => row.id === old.document.id)?.deletion_batch_id).toBe(oldDelete.batchId)

    const repeated = await run(Effect.gen(function* () {
