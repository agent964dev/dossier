...
const protectedRoutes = [
  { name: 'load workspace', method: 'GET', path: '/api/workspace' },
  {
    ...
    body: {
      kind: 'email',
      value: 'allowed@workspace-api.example',
      role: 'member',
    },
  },
  {
...
describe('workspace management HTTP API', () => {
  it('returns admin member and allowlist projections without other workspaces', async () => {
    ...
    const member = await scopedPrincipal(
      'workspace_api_shapes_member',
      admin.workspaceId,
    )
    const outsider = await seedPrincipal(env, {
      suffix: 'workspace_api_shapes_outsider',
    })
    await env.DB.batch([
      env.DB.prepare(
        ...
        'allow_workspace_api_shapes',
        'admin@workspace-shapes.example',
        admin.workspaceId,
        admin.accountId,
        now,
      ),
      env.DB.prepare(
        ...
        'allow_workspace_api_other_shapes',
        'other-workspace-shapes.example',
  ...
  it.each(protectedRoutes)(
    'rejects members who try to $name with editor_required',
    async (route) => {
      const member = await seedPrincipal(env, {
        suffix: `workspace_api_forbidden_${route.name.replaceAll(' ', '_')}`,
      })
      await expectError(
        await request(route.path, { ...route, token: member.token }),
        403,
        'editor_required',
      )
    },
  ...
  it.each(protectedRoutes)(
    'rejects anonymous callers who try to $name with 401',
    async (route) => {
  ...
  it('allows a deployment-admin service account with no workspace membership', async () => {
    ...
    const added = await request('/api/workspace/allowlist', {
      ...
      body: {
        kind: 'domain',
        value: 'deployment-admin-workspace.example',
        role: 'member',
      },
    })
  ...
  it.each([
    {
      kind: 'email',
      value: 'Person@Workspace-Duplicate.Example',
      normalized: 'person@workspace-duplicate.example',
      role: 'admin',
    },
    {
      kind: 'domain',
      value: 'Workspace-Duplicate.Example',
      normalized: 'workspace-duplicate.example',
      role: 'member',
    },
  ] as const)(
    'adds normalized $kind entries and rejects local and cross-workspace duplicates',
    async ({ kind, value, normalized, role }) => {
      const admin = await seedPrincipal(env, {
        suffix: `workspace_api_duplicate_${kind}`,
        role: 'admin',
      })
      const other = await seedPrincipal(env, {
        suffix: `workspace_api_duplicate_other_${kind}`,
        role: 'admin',
      })
      const added = await request('/api/workspace/allowlist', {
        token: admin.token,
      ...
      for (const token of [admin.token, other.token]) {
        await expectError(
          await request('/api/workspace/allowlist', {
            token,
            body: {
              kind,
              value: normalized,
  ...
  it('removes an allowlist entry, returns 404 on repeat, and preserves its existing member', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_allow_remove',
      role: 'admin',
    })
    const member = await scopedPrincipal(
      'workspace_api_allow_remove_member',
      admin.workspaceId,
      {
    ...
    const added = await request('/api/workspace/allowlist', {
      ...
      body: {
        kind: 'email',
        value: 'member@workspace-remove-entry.example',
        role: 'member',
      },
    })
    expect(added.status).toBe(200)
    const entry = await env.DB.prepare(
      'SELECT id FROM allowlist WHERE workspace_id = ?',
    )
      .bind(admin.workspaceId)
      .first<{ id: string }>()
    ...
    await expectError(
      await request(path, { token: admin.token, method: 'DELETE' }),
      404,
      'not_found',
    )
    expect(await membership(member)).toEqual({ role: 'member' })
    expect(
      await env.DB.prepare('SELECT id FROM allowlist WHERE id = ?')
        .bind(entry!.id)
        .first(),
    ).toBeNull()
    const upload = await request('/api/uploads', {
      token: member.token,
      body: {
        html: '<!doctype html><html><head><title>Still a member</title></head><body>Allowed</body></html>',
      },
    })
  ...
  it('does not delete an allowlist entry from another workspace', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_allow_scope',
      role: 'admin',
    })
    const other = await seedPrincipal(env, {
      suffix: 'workspace_api_allow_scope_other',
      role: 'admin',
    })
    const added = await request('/api/workspace/allowlist', {
      token: other.token,
      body: {
        kind: 'domain',
        value: 'workspace-delete-scope.example',
        role: 'member',
      },
    })
    expect(added.status).toBe(200)
    const entry = await env.DB.prepare(
      ...
      .bind(other.workspaceId)
      .first<{ id: string }>()
    expect(entry).not.toBeNull()
    await expectError(
      await request(`/api/workspace/allowlist/${entry!.id}`, {
        token: admin.token,
        method: 'DELETE',
      }),
      404,
  ...
  it('promotes and demotes a member while retaining another admin', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_roles',
      role: 'admin',
    })
    const member = await scopedPrincipal(
      'workspace_api_roles_member',
      admin.workspaceId,
    )
    for (const role of ['admin', 'admin', 'member'] as const) {
      const response = await request(
        `/api/workspace/members/${member.accountId}`,
        {
          token: admin.token,
          body: { role },
        },
      ...
      const management = await request('/api/workspace', {
        token: member.token,
      })
  ...
  it('rejects demotion of the last admin with 409 without changing membership', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_last_demote',
      role: 'admin',
    })
    await expectError(
      await request(`/api/workspace/members/${admin.accountId}`, {
        token: admin.token,
        body: { role: 'member' },
      }),
      409,
  ...
  it('rejects removing yourself even when another admin remains', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_self_remove',
      role: 'admin',
    })
    await scopedPrincipal(
      'workspace_api_self_remove_other',
      admin.workspaceId,
      { role: 'admin' },
    )
    await expectError(
  ...
  it('rejects a deployment admin removing the last workspace admin', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_last_remove',
      role: 'admin',
    })
    const service = await scopedPrincipal(
      'workspace_api_last_remove_service',
      admin.workspaceId,
      {
        role: null,
        accountKind: 'service',
  ...
  it('removes a member and immediately rejects uploads through their still-valid API key', async () => {
    const admin = await seedPrincipal(env, {
      suffix: 'workspace_api_member_remove',
      role: 'admin',
    })
    const member = await scopedPrincipal(
      'workspace_api_member_remove_target',
      admin.workspaceId,
    )
    ...
    await expectError(
      await request('/api/uploads', {
        token: member.token,
        body: {
          html: '<!doctype html><html><head><title>Removed member</title></head><body>Forbidden</body></html>',
        },
      }),
      403,
      'publisher_required',
  ...
  it.each(['POST', 'DELETE'])(
    'returns 404 for %s targeting a member of another workspace',
    async (method) => {
      const admin = await seedPrincipal(env, {
        suffix: `workspace_api_member_scope_${method}`,
        role: 'admin',
      })
      const other = await seedPrincipal(env, {
        suffix: `workspace_api_member_scope_other_${method}`,
      })
      await expectError(
        await request(`/api/workspace/members/${other.accountId}`, {
          token: admin.token,
          method,
  ...
  it.each([
    {
      name: 'missing kind',
      body: { value: 'person@workspace-invalid.example', role: 'member' },
    },
    {
      name: 'invalid kind',
      body: {
        kind: 'group',
        value: 'workspace-invalid.example',
        role: 'member',
      },
    },
    {
      name: 'non-string value',
      body: { kind: 'email', value: 42, role: 'member' },
    },
    {
      name: 'invalid role',
      body: {
        kind: 'email',
        value: 'person@workspace-invalid.example',
        role: 'owner',
      },
    },
    {
    ...
    },
    {
      ...
      body: { kind: 'email', value: 'not-an-email', role: 'member' },
    },
    {
      name: 'invalid domain syntax',
      body: { kind: 'domain', value: 'bad domain.example', role: 'member' },
    },
    {
      name: 'email kind with domain value',
      body: {
        kind: 'email',
        value: 'workspace-invalid.example',
        role: 'member',
    ...
    {
      name: 'domain kind with email value',
      body: {
        kind: 'domain',
        value: 'person@workspace-invalid.example',
        role: 'member',
    ...
    'rejects allowlist $name with 422 policy_rejected',
    async ({ name, body }) => {
      const admin = await seedPrincipal(env, {
        suffix: `workspace_api_invalid_${name.replaceAll(' ', '_')}`,
        role: 'admin',
      })
      await expectError(
        await request('/api/workspace/allowlist', {
  ...
  it.each([
    ...
    'rejects member mutation $name with 422 policy_rejected',
    async ({ name, body }) => {
      const admin = await seedPrincipal(env, {
        suffix: `workspace_api_invalid_role_${name.replaceAll(' ', '_')}`,
        role: 'admin',
      })
      await expectError(
        await request(`/api/workspace/members/${admin.accountId}`, {
  ...
  it.each(['allowlist', 'members'])(
    'rejects malformed JSON for %s with 422 policy_rejected',
    async (resource) => {
      const admin = await seedPrincipal(env, {
        suffix: `workspace_api_json_${resource}`,
        role: 'admin',
      })
      const path =
        resource === 'allowlist'
      ...
      await expectError(
        await request(path, { token: admin.token, rawBody: '{' }),
        422,
        'policy_rejected',
      )
    },
