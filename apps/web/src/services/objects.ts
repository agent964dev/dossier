...
export interface ObjectsService {
  ...
  readonly get: (
    key: string,
  ) => Effect.Effect<R2ObjectBody | null, StorageError>
    ...
    put: (key, value, options) =>
      Effect.tryPromise({
        try: () => bucket.put(key, value, options),
          ...
          new StorageError({ operation: `R2 put ${key}`, cause }),
      }),
    get: (key) =>
      Effect.tryPromise({
        try: () => bucket.get(key),
        catch: (cause) =>
          new StorageError({ operation: `R2 get ${key}`, cause }),
      }),
    head: (key) =>
      Effect.tryPromise({
        try: () => bucket.head(key),
        catch: (cause) =>
          new StorageError({ operation: `R2 head ${key}`, cause }),
      }),
    delete: (key) =>
      Effect.tryPromise({
        try: () => bucket.delete(key),
        catch: (cause) =>
          new StorageError({ operation: `R2 delete ${key}`, cause }),
      }),
  }
}
