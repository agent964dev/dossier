...
describe('validateHtml', () => {
  ...
  it('keeps upstream errors for empty documents and blocked tags', () => {
    expect(validateHtml('   ', OPTIONS).errors).toEqual(['HTML document is empty.'])
  ...
  it('counts UTF-8 bytes with TextEncoder, including the BOM', () => {
    ...
    expect(validateHtml(fixture, { ...OPTIONS, maxBytes: bytes - 1 })).toEqual({
      ok: false,
      errors: [`HTML document is ${bytes} bytes; maximum is ${bytes - 1} bytes.`],
      warnings: [],
      title: null,
      stats: {
  ...
  it('checks blocked content inside template and both noscript parse modes', () => {
    const result = validateHtml(
      html('<template><form></form></template><noscript><object></object></noscript>'),
      OPTIONS,
    )

    expect(result.errors).toEqual(
      expect.arrayContaining(['Blocked <form> tag found.', 'Blocked <object> tag found.']),
    )
  ...
  it('allows workspace stylesheet paths and records their references', () => {
    const result = validateHtml(
      html('', '<title>Styles</title><link rel="stylesheet" href="/a/theme.css"><link rel="stylesheet" href="/a/theme@2.css">'),
      OPTIONS,
    )

    expect(result.ok).toBe(true)
    expect(result.stats.stylesheetRefs).toEqual(['/a/theme.css', '/a/theme@2.css'])
  })
  ...
  it('allows same-origin document iframes and gates external iframe hosts', () => {
    expect(validateHtml(html('<iframe src="/d/abcdefghijkl"></iframe>'), OPTIONS).ok).toBe(
      true,
    )

    const youtube = html('<iframe src="https://youtube.com/embed/abc"></iframe>')
  ...
  it('requires allowlisting and usable integrity for external classic scripts', () => {
    ...
    expect(
      validateHtml(fixture, { ...OPTIONS, scriptHostAllowlist: ['cdn.example'] }).ok,
    ).toBe(true)
    ...
    for (const integrity of ['not-a-hash', 'md5-ZXhhbXBsZQ==', 'sha384-example']) {
      const malformed = validateHtml(
        html(`<script src="https://cdn.example/app.js" integrity="${integrity}"></script>`),
        { ...OPTIONS, scriptHostAllowlist: ['cdn.example'] },
      )
  ...
  it('rejects SVG script href and xlink:href sources', () => {
    for (const source of [
      ...
      const result = validateHtml(
        html(`<svg xmlns:xlink="http://www.w3.org/1999/xlink"><script ${source}></script></svg>`),
        OPTIONS,
      )
      expect(result.errors).toContain('Externally sourced SVG scripts are not allowed.')
      expect(result.stats.hasInlineScript).toBe(false)
    }
  })

  it('rejects non-classic script types while allowing inline classic scripts', () => {
    expect(validateHtml(html('<script type="module">export {}</script>'), OPTIONS).errors).toContain(
      'Unsupported script type "module" found.',
    )
    expect(validateHtml(html('<script type="text/javascript">void 0</script>'), OPTIONS).ok).toBe(
      true,
    )
  })
  ...
  it('runs only static rules when server allowlists and limits are unavailable', () => {
    ...
    expect(
      validateHtmlStatic(html('<style>.x { background: url("http://images.example/x") }</style>'))
        .ok,
    ).toBe(false)
  })
  ...
  it('applies CSS validation to style tags and style attributes', () => {
    const styleTag = validateHtml(
      html('<style>.x { background: url("https://evil.example/pixel") }</style>'),
      OPTIONS,
    )
  ...
  it('rejects nested image-set var() fallback strings in style tags and attributes', () => {
    const fallback = 'image-set(var(--missing, &quot;https://blocked.invalid/x.png&quot;) 1x)'
    expect(
      validateHtml(html(`<style>.x { background: ${fallback.replaceAll('&quot;', '"')} }</style>`), OPTIONS)
        .errors,
    ).toContain('Blocked unsafe CSS in <style> tag.')
    expect(validateHtml(html(`<div style="background: ${fallback}"></div>`), OPTIONS).errors).toContain(
...
describe('validateCss', () => {
  ...
  it.each(adversarialFixtures)('rejects adversarial destination fixture %#', (fixture) => {
  ...
  it('allows /a destinations and HTTPS destinations on the style host allowlist', () => {
    ...
    expect(
      validateCss('.x { background: url("https://images.example/pixel.png") }', {
        ...OPTIONS,
  ...
  it('rejects expression(), behavior:, javascript URLs, and suspicious Raw recovery nodes', () => {
    expect(validateCss('.x { width: expression(alert(1)) }', OPTIONS).errors).toContain(
      'Blocked unsafe CSS expression().',
    )
    expect(validateCss(String.raw`.x { beh\61vior: url("/a/x.htc") }`, OPTIONS).errors).toContain(
      'Blocked unsafe CSS behavior property.',
    )
    expect(validateCss('.x { background: url("javascript:alert(1)") }', OPTIONS).errors).toContain(
      'Blocked unsafe CSS URL.',
    )
    expect(validateCss('.x { background: url(javascript:alert(1)) }', OPTIONS).errors).toContain(
